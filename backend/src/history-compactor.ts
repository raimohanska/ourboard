...
export async function doIt() {
    ...
    for (let id of ids) {
        await inTransaction(async (client) => {
            ...
            if (bundles.length == 1) {
                console.log(
                    `Board ${id}: 1 bundle containing ${bundles[0].events.events.length} events => no need to compact`,
                )
            } else {
                ...
                if (consistent) {
                    ...
                    if (snapshot.serial) {
                        ...
                        if (followingEvents.length > 0 && followingEvents[0].serial !== snapshot.serial + 1) {
                            ...
                            await bootstapHistory(id, snapshot, followingEvents, client)
                        }
                    } else {
                        console.warn(
                            "No snapshot with serial available discarding history and bootstrapping to latest state.",
                        )
                        await bootstapHistory(id, mkSnapshot(snapshot, 0), [], client)
                    }
                }
            }
        })
...
async function bootstapHistory(boardId: Id, snap: Board, events: BoardHistoryEntry[], client: PoolClient) {
